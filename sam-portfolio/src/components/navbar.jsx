    ...
    <div className='h-full flex items-center justify-between px-4 sm:px-8 md:px-12 lg:px-20 xl:px-48 text-xl'>
      ...
      <div className='md:hidden lg:flex flex w-1/3'>
        <Link
          href='/'
          className={
            'text-sm bg-black opacity-50 rounded-md p-1 font-semibold flex items-center justify-center transition duration-300 ease-in-out transform hover:opacity-100 hover:scale-105'
          }
        >
          <Image
      ...
      {/* PAGE LINKS */}
      <div
        className={`hidden md:flex gap-4 w-1/3 justify-center ${
          isHome ? 'text-neutral-900' : 'text-neutral-300'
        }`}
      >
        {links.map(link => (
      ...
      <div className='hidden md:flex gap-4 w-1/3 justify-end'>
        <Link href='https://github.com/srichard3' target='_blank'>
          <Image
            ...
            height={24}
            alt='GitHub'
            className={`transition duration-100 ease-in-out transform hover:opacity-100 hover:scale-110 ${
              isHome ? 'opacity-50' : 'opacity-100'
            }`}
          />
        </Link>
        <Link
